...
TEST(MethodChecking, mapNetwork2Local1NonLocal) {
    ...
    std::string defaultGateway {Tools::getDefaultGateway()};
    InternalInterface interfaceWithGateway = myMachine.getInterfaceFromSubnet(defaultGateway, AF_INET);
    IPv4Range rangeLocal2 {IPv4Range(interfaceWithGateway.getIPAddress(), networkMask, myMachine)}; //Local Range 2
...
TEST(MethodChecking, mapNetworkGreaterThanLocalNetwork) {
    LocalHost myMachine {LocalHost(true)};
    ...
    IPv4Range range {interfaceWithGateway.getIPAddress(), networkMask, myMachine}; //Local Range 1

    Mapper mapper {AF_INET};
    auto result = mapper.mapNetwork(range.getIPsNonLocal(), range.getIPsLocal(), myMachine);
    EXPECT_GE(result.size(), 1); // At least the gateway responds
}