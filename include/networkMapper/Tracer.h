...
class Tracer {
    ...
    static TraceRouteResult trace(std::string& destination, uint16_t numOfHops, RawSocket& socket);
...
public:
    ...
    static std::vector<TraceRouteResult> multipleTraces(std::vector<std::string>& destinations, uint16_t numOfHops = 64);
};
